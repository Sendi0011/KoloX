...
export function useKoloContract() {
  ...
  const createKolo = useCallback(async (data: CreateKoloFormData, userAddress: string): Promise<number | null> => {
    ...
    setError(null)
    try {
      console.log('[kolox] Creating Kolo with data:', data)
      ...
      setError(message)
      console.error('[kolox] Create Kolo error:', message)
      return null
    } finally {
      setLoading(false)
    }
  ...
  const sendSTX = useCallback(async (recipientAddress: string, amount: number, userAddress: string): Promise<boolean> => {
    ...
    setError(null)
    try {
      console.log('[kolox] Sending STX:', amount, 'to', recipientAddress)
      ...
      setError(message)
      console.error('[kolox] STX transfer error:', message)
      return false
    } finally {
      setLoading(false)
    }
  ...
  const getKolo = useCallback(async (koloId: number): Promise<Kolo | null> => {
    ...
    setError(null)
    try {
      console.log('[kolox] Fetching Kolo:', koloId)
      ...
      setError(message)
      console.error('[kolox] Fetch Kolo error:', message)
      return null
    } finally {
      setLoading(false)
    }
